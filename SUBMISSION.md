...
| T14 |  |  |  |
| T15 |  | Flag read at build time from `process.env.FEATURE_SHOW_INSIGHTS` in `generate-status.mjs` (starter snippet placed there verbatim as `featureFlags()`) and separately from `import.meta.env.VITE_FEATURE_SHOW_INSIGHTS` in a new `team-site/src/components/Insights.tsx`, wired into `App.tsx`. Both env vars are sourced in `ci.yml`'s build env from the single `FEATURE_FLAG_NAME` GitHub secret — no value hardcoded, no secret printed. `/status.features` shows only `{ task, showInsights: boolean, valueRedacted: true }`. | Two env var names (`FEATURE_SHOW_INSIGHTS` for the Node build script, `VITE_FEATURE_SHOW_INSIGHTS` for the client bundle) both read from the same `FEATURE_FLAG_NAME` secret, mirroring the existing `WEB3FORMS_ACCESS_KEY` → `VITE_WEB3FORMS_ACCESS_KEY` pattern from T10. Verified locally by building twice: with the vars unset, `/status.features.showInsights` is `false` and the built JS bundle contains no "Cohorts needing attention" string (grep on `dist/assets/*.js`); with both vars set to `true`, the flag flips to `true` and the string appears in the bundle. `Insights.tsx` carries an `AI-REVIEW-MARKER` comment per `AGENTS.md`'s standing instruction — remove it during review before merging (same as T11/T13). |
| T16 |  |  |  |
| T17 |  |  |  |
| T18 |  |  |  |
| T19 |  |  |  |
