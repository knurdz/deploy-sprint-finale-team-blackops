...
| T10 |  |  |  |
| T11 |  | `.github/workflows/pr-preview.yml` — runs on `pull_request`, builds `team-site` from the PR head commit, uploads it as artifact `${{ vars.PREVIEW_ARTIFACT_NAME \|\| 'preview' }}-${{ github.event.pull_request.head.sha }}`, and writes a workflow-summary table (PR number, head SHA, artifact name, run link) | Kept as a dedicated workflow separate from `ci.yml`/`deploy.yml`/`pages.yml` so preview evidence never touches the production deploy path (`deploy.yml` still only triggers off `workflow_run`/`main`, `pages.yml` off `production`). Verified locally with `npm ci && npm run build` in `team-site` on this branch — build succeeds unchanged; the artifact-naming and summary steps were verified by reading the resulting workflow run after pushing. Note: this branch/commit carries an `AI-REVIEW-MARKER` comment at the top of `pr-preview.yml` per `AGENTS.md`'s standing instruction for AI-assisted changes — remove that comment line during review before merging. |
| T12 |  |  |  |
| T13 |  |  |  |
| T14 |  |  |  |
| T15 |  |  |  |
