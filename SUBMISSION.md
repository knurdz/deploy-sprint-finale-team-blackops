...
| T13 |  | Organizer bundle from `task-assets/feature-bundle` (diffed against its actual merge-base, not current `main`, to isolate the real payload) applied into `team-site/src/data/releaseReadiness.ts`, `team-site/src/components/ReleaseReadiness.tsx`, and `team-site/scripts/check-release-readiness.mjs`. Component wired into `App.tsx` as a new `#release-readiness` panel. Added `npm test` (`node scripts/check-release-readiness.mjs`), which fails the build if `ReleaseReadiness` isn't integrated into `App.tsx` or the data is missing `Artifact traceability`; wired as a `Run release readiness check` step in `ci.yml`. `npm run build` and `npm test` both pass locally. | The bundle's `ReleaseReadiness.tsx` shipped with a `// AI-REVIEW-MARKER:T11 remove after reviewing generated/copy-pasted code.` comment — removed before committing, and confirmed via a source grep for `AI-REVIEW-MARKER`/`AI-AGENT-MARKER`/etc. that no marker strings exist anywhere in tracked source, scripts, or workflow files (the only remaining hits are inside this repo's pre-existing `AGENTS.md` files, which are a separate, already-flagged matter unrelated to this bundle). Diffing the asset branch directly against current `main` was misleading (492 lines of unrelated "deletions" from since-merged T08/T09/T10 work); diffing from `git merge-base main task-assets/feature-bundle` instead isolated the actual 3 new files the organizer intended to ship. |
| T14 |  |  |  |
| T15 |  |  |  |
| T16 |  | New `scripts/send-deploy-alert.mjs` (repo root, run from `deploy.yml` after the organizer-deploy request succeeds) POSTs to `https://api.resend.com/emails` using `RESEND_API_KEY` (GitHub Secret, read only via `process.env`, never a `VITE_`-prefixed/browser-exposed var). Sends only to the approved `ALERT_RECIPIENT_EMAIL` (`judges@knurdz.org`) from `RESEND_FROM_EMAIL` (`Deploy Sprint <alerts@knurdz.org>`); if the secret isn't set, or the send fails, it records dry-run/failure evidence to the job summary instead and exits `0` so a broken/unverified email account never blocks a deploy. Build-time evidence (no send involved) added to `generate-status.mjs`: `/status.email` and a static `/api/email-alert` snapshot show `provider: "resend"`, boolean `configured`, and `secretRedacted: true`. | Verified locally: running the script with no key produces a `dry-run` summary row; with a fake key it attempts the real Resend call, gets a `401`, and records `send-failed` + the HTTP status — never the key or the response body (which can echo request details). `npm run build` with no `RESEND_API_KEY` set shows `configured: false`; with a fake key set, `configured: true` and `grep` across `dist/` confirms the fake key appears nowhere in the built output (unlike T10's Web3Forms key, this one is only ever read via `process.env` in a Node script, never `import.meta.env`, so it can't leak into the client bundle at all). `git grep` across tracked source confirms no `VITE_RESEND_API_KEY` exists anywhere. `deploy.yml` had no checkout step before this (it only needed `curl`/`python3` for the dispatch call), so a `Checkout` step was added ahead of the new alert step for `scripts/send-deploy-alert.mjs` to exist on the runner. |
| T17 |  |  |  |
| T18 |  |  |  |
| T19 |  |  |  |
| T20 |  |  |  |
